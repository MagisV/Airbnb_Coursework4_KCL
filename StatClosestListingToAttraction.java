    ...
    protected void updateStatistic(ArrayList<AirbnbListing> listings)
    {
        ...
        if (!listings.isEmpty()) {
            for (Map.Entry<String, double[]> location : locationsList.entrySet()) {
                ...
                result.append(location.getKey()).append(": ").append("\n").append(currentResult.getKey().getName()).append(" ").append(currentResult.getValue()).append("m\n"); // Get the name of the original location, the nearest apartment and the distance in meters
            }
        }
        else result = new StringBuilder("No properties fit Your search.");
        statLabel.setText(result.toString());
    }
