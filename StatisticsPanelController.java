...
public class StatisticsPanelController extends MainframeContentPanel {
    ...
    {
        Statistic accommodationType, availableProperties, averageNumReviews, mostExpensiveBorough,
     ...
     * @param currentPanel The panel the action is applied on.
     * @param removedStat The statistic that was being displayed and is now removed.
     * @param displayedStat The statistic that is now being displayed.
     */
    private void updateOtherPanels(StatsPanelElement currentPanel, Statistic removedStat, Statistic displayedStat) {
        for (StatsPanelElement allStatPanel : allStatPanels) {
            if (allStatPanel != currentPanel)
                allStatPanel.updateList(removedStat, displayedStat); // The previously current stat of the panel is now free. Add it to the list of other panels. The nextStat is now taken, remove it from the list of the other panels.
        }
    }
